...
// SQL commands for Profile table (UserTable)
const insertProfile = "INSERT into UserTable (userId, firstName) values (?,?)";
const updateProfile = "update UserTable set firstName = ? where userId = ?";
const getProfileIDs = "SELECT * FROM UserTable where userId = ?";
const getAllProfileIDs = "SELECT * from UserTable";
const getNameFromUserID = "SELECT firstName from UserTable where userId = ?";
...
async function post_activity(activity, userId) {
  try {
    let list = act.ActivityToList(activity);
    list.push(userId);
    await db.run(insertDB, list);
  } catch (error) {
...
// Function to retrieve user data from Google

/*
Function to update a new user in our profile table.
Adds the profile to UserTable if is a new profile.
*/
async function update_user(userID, name) {
  try {
    // Check if the profile already exists or not
    var profileIDs = await db.all(getProfileIDs, [userID]);
    //if (profileIDs != undefined && profileIDs.includes(userID)) {
    if (profileIDs != undefined && profileIDs.length > 0) { 
      console.log("Profile already exists in user table");
    }
    else {
      console.log("Add profile to user table");
      await db.run(insertProfile, [userID, name]);
    }
...
/*
Prints out all Users for debugging purposes.
*/
async function printAllUsers() {
  var profileIDs = await db.all(getAllProfileIDs, []);
  console.log("ProfileIDs are ", profileIDs);
}

/*
Retrieves the FirstName of the user when given the UserID.
*/
async function getNameFromID(userID) {
  var nameObj = await db.get(getNameFromUserID, [userID]);
  var name = nameObj.firstName;
  return name;
}
