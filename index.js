...
app.get('/all', isAuthenticated, async function(request, response, next) {
  ...
  app.post('/store', isAuthenticated, async function(request, response, next) {
    ...
    await dbo.post_activity(activity, parseInt(request.user.userData))
  ...
  app.get('/reminder', isAuthenticated, async function(request, response, next) {
    ...
    let result = await dbo.get_most_recent_planned_activity_in_range(0, currTime, parseInt(request.user.userData))
    await dbo.delete_past_activities_in_range(0, currTime, parseInt(request.user.userData));
  ...
  app.get('/week', isAuthenticated, async function(request, response, next) {
    ...
    if (activity === undefined) {
      let result = await dbo.get_most_recent_entry(parseInt(request.user.userData))
      try {
    ...
    let max = date
    let result = await dbo.get_similar_activities_in_range(activity, min, max, parseInt(request.user.userData))
...
/*
Displays the user's first name in Welcome page.
*/
app.get('/name', isAuthenticated, async function(request, response, next) {
  let username = await dbo.getNameFromID(parseInt(request.user.userData));
  
  response.send({firstName: username});
});

/*
Logs out the current user and redirects to main splash page.
*/
...
function gotProfile(accessToken, refreshToken, profile, done) {
    ...
    dbo.update_user(userid, firstname);
    dbo.printAllUsers();
